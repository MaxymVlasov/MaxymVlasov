...
When all hope was gone that our lovely workflow with `**/.terraform.lock.hcl` in `.gitignore` won't backfire, I chose to try to add `.terrafrom.lock.hcl` to all our 289 root modules. You may ask:

## Why are these lockfiles needed?

Well, except "highly recommended by Hashicorp" means that they force you to use lockfiles, here are a few additional reasons why you would like to use them - Repeatability and Security.
...
* There is a breaking change in a minor release of the provider because they _forgot_ to make a major release
...
There are these `h1` and `zh` [hashes](https://developer.hashicorp.com/terraform/language/files/dependency-lock#hashes) inside `.terrafrom.lock.hcl`:
...
It decreases the probability of the [supply chain attack](https://en.wikipedia.org/wiki/Supply_chain_attack) the weakest link in your supply chain is terraform providers.
...
No, you cannot skip this step if you have a huge terraform codebase: almost certainly there is something broken.
...
3. If you have huge repo - uncomment this line and specify the correct `# files: '^path/to/your/terraform/root/folder/[a-c]'`  
    ...
    `files` and `exclude` uses a Python `re.search` regex ([docs](https://pre-commit.com/#regular-expressions)). By specifying `[a-c]` at the end, one can limit the number of directories that should be processed by a single run
...
4. If you also encounter these problems, modify `.github/.pre-commit-tf-lockfiles.yaml` and rerun `pre-commit` until everything is Ok:
    ...
    > **Note**: To save some time, you may need to comment out the last hook section for future lockfile generation.
...
2. Add a GitHub workflow, which installs all the dependencies and run `pre-commit run` every morning. It creates a new PR in the Renovate style:
...
* Newly generated repository [actions/typescript-action](https://github.com/actions/typescript-action) contains

    ![Newly generated repo dependabot PR](images/2023-06-15-renovate/newly-generated-repo.jpg)

* Dependabot PR is there for 2 weeks!

    ![How long can a dependabot PR live without any attention?](images/2023-06-15-renovate/github-dependabot-alert-long-live.png)
...
* For now, Renovate cannot resolve [`!=`](https://developer.hashicorp.com/terraform/language/expressions/version-constraints#version-constraint-syntax) version constrain ([renovate/#22417](https://github.com/renovatebot/renovate/issues/22417)), so it just fails to create a PR if at least one `!=` exists in a repository.
...
It adds a basic `.terrafrom.lock.hcl`, which you can commit as it is and wait for the next lockfile update job.
...
* For a better user experience, you need a source of truth (automated `terraform plan` in CI/Terratest, etc.) which shows that changes do not break anything. It can be [Atlantis](https://github.com/runatlantis/atlantis), [Spacelift](https://spacelift.io/), [Terraform Cloud](https://www.hashicorp.com/products/terraform), or you can do it in your own CI.
